...
# 41→# 将字符串转换为布尔值
# 42→# 是否强制推送信息
# 43→# 若设置了环境变量，则使用环境变量的值；否则若是非GitHub Actions环境,则默认强制推送信息
if force_push_message is not None:
    force_push_message = force_push_message == "True"
else:
    force_push_message = not github_actions

# 49→# 定义文件路径
# 支持通过环境变量自定义数据目录，方便多用户隔离
folder_path = os.environ.get("DATA_DIR", "data")
